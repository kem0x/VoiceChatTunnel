...
void ImGui::TextStyled(const char* text, float fontSize, ImU32 color)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    window->DrawList->AddText(g.Font, fontSize, window->DC.CursorPos, color, text, nullptr, 0);
...
bool ImGui::ClickableText(const char* string)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    ImVec2 textSize = CalcTextSize(string);
...
bool ImGui::Spinner(const char* label, float radius, float thickness, const ImU32& color)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    int num_segments = 30;
    int start = abs((long)(ImSin((float)g.Time * 1.8f) * (num_segments - 5)));
    ...
    for (int i = 0; i < num_segments; i++)
    {
        ...
        window->DrawList->PathLineTo(ImVec2(centre.x + ImCos(a + (float)g.Time * 8.0f) * radius, centre.y + ImSin(a + (float)g.Time * 8.0f) * radius));
    }
